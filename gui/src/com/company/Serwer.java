...
public class Serwer {
    ...
    private static String path = "E:/nircmd-x64/nircmd.exe";
...
//	Wystarczy podac sama nazwe pliku w stringu, bez .exe
    ...
    public static void runProgram(String appName) {
        try {
            pr = rt.exec(path + " exec show " + appList.get(appName));
    ...
    //Przykladowe zmienne dla serwera
    private static ServerSocket server;
    private static int port = 9876;

    public static void printVars(Gui ginterface){
        ginterface.clearVarsView();
        for (String key : commandList.keySet()) ginterface.listCommands(key + " -> " + commandList.get(key));
        for (String key : appList.keySet()) ginterface.listApps(key + " -> " + appList.get(key));
        for (String key : pathList.keySet()) ginterface.listPaths(key + " -> " + pathList.get(key));
    }

    public static void main(String[] args) throws Exception {
        //Ladowanie ustawien
        loadProperties();
        Gui ginterface = new Gui();
        ginterface.show();
        System.out.println("Dostępne komendy w oparciu o zapisane ustawienia:");
        for (String key : commandList.keySet()) {
            System.out.print(key + ": ");
            System.out.println(commandList.get(key));
        }

//        wywołaj f
        printVars(ginterface);

        //Przykladowa instancja serwera
        ginterface.logNetInfo("Tworzę połączenie...");
        server = new ServerSocket(port);
        ginterface.logNetInfo("OK\n");
        ginterface.logNetInfo("Nasłuchiwanie:\n");
        ginterface.logNetInfo("-> IP: " + server.getInetAddress().toString() + "\n");
        ginterface.logNetInfo("-> Port: " + port + "\n");
        ginterface.logNetInfo("Log komend:\n");


        while (true) {
            System.out.println("\nOczekiwanie na wiadomość klienta");
            ...
            ginterface.logNetInfo("Otrzymana wiadomość: " + message + "\n");

            //Wybor zadania na podstawie wejsciowego stringa
